...
export class ProjectController {
  ...
  async joinProject(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Check if project is in a joinable state
      if (project.status === 'completed' || project.status === 'paused') {
        throw createError(`Cannot join project in ${project.status} state`, 400);
      }

      // Join the project
      const updatedProject = await projectService.joinProject(id, userId);
